...
### Нагрузка на руку
![nagryzka_na_ryky](D:/Документы/GitHub/keyboard_typing/docs/source/_static/nagryzka_na_ryky.png)
...
### 🟦 Общая нагрузка
- 🟢 **Индексы** (левый и правый) — по 15% нагрузки: самые активные и сильные пальцы.
- 🟡 **Средние пальцы** — по 9%: участвуют активно, но не перегружены.
- 🔴 **Мизинцы и большие пальцы** — от 5% до 9%: нагрузка умеренная, что говорит о сбалансированном распределении.
- 🔸 `Йцукен`, `Скоропись`, `Зубачев`, `Ант`, `Фонетическая`, `Диктор` — по 15%: тестировались равномерно.
- 🔸 `Вызов` — 10%: либо менее эффективна, либо использовалась реже.

### ⚖️ Нагрузка на руку
-  Почти все раскладки демонстрируют **почти идеальный баланс**:
- `Йцукен`, `Вызов`, `Фонетическая` — 51% левая / 49% правая
- `Диктор`, `Ант`, `Зубачев`, `Скоропись` — ровно 50% / 50%
