...
class KeyboardAnalyzer:
    ...
    def analyze_combinations(self, text):
        """
        Анализирует комбинации букв в тексте по новой логике.

        Функция очищает входной текст, оставляя только русские буквы, 
        разбивает его на слова и выполняет анализ биграмм, триграмм и 
        четырехграмм с учетом расположения букв на руках и пальцах.

        Логика анализа:
            - Биграммы (2 буквы): анализируются все пары подряд идущих букв.
              Если буквы печатаются разными руками — комбинация считается неудобной.
              Если одной рукой — оценивается направление движения.
            - Триграммы (3 буквы): анализируются только если все буквы печатаются одной рукой.
              При смене руки внутри тройки комбинация не учитывается.
            - Четырехграммы (4 буквы): аналогично триграммам, анализируются только при печати одной рукой.
              При смене руки внутри четверки комбинация не учитывается.

        Args:
            text (str): Входной текст для анализа, содержащий русские буквы.

        Returns:
            dict: Словарь с результатами анализа, включающий количество удобных,
                  частично удобных и неудобных комбинаций для биграмм, триграмм и четырехграмм.
        """
